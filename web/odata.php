 ...
 * (UI/on-demand + nightly via odata_get_all) naar Mímir i.p.v. BC.
 *
 * Met $mimirApi gezet zijn $auth_list / $environment / $baseUrl / $auth ongebruikt voor BC;
 * Mímir beheert environments — Demeter heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
 ...
 *
 * max_age-beleid: nightly.php gebruikt DEMETER_NIGHTLY_MAX_AGE (14400);
 * UI/on-demand houdt bestaande TTLs (o.a. index.php: 12 uur).
 * Geen hourly.php in Demeter.
 */

/** Mímir max_age for nightly.php fetches (4h). */
const DEMETER_NIGHTLY_MAX_AGE = 14400;
...
function odata_mimir_request(string $method, string $path, ?array $jsonBody = null): array
{
    ...
    $opts = [
        ...
        CURLOPT_USERAGENT => 'Demeter-MimirClient/1.0',
    ];
    if ($jsonBody !== null) {
...
if (odata_mimir_enabled()) {
    // Demeter may set $environment as string or list; only fill when completely unset/empty string.
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        ...
        $rows = odata_mimir_fetch_all($url, $ttlSeconds === 0 ? 3600 : $ttlSeconds);
    ...
    if (is_file($cachePath)) {
        ...
        if ($cached['valid']) {
            ...
            odata_call_time_log_record($url, $cacheDurationMs, true, count($cached['data']));
    ...
    odata_call_time_log_record($url, $queryDurationMs, false, count($all));
